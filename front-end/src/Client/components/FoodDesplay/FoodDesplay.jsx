...
const FoodDisplay = () => {
  ...
  const { data: food_list = [], isLoading: productsLoading, error: productsError } = useQuery({
    queryKey: ["products"],
    queryFn: fetchingProducts,
  });
    const filtredlist=food_list?.filter(i=>i.category_name.toLowerCase() !== 'extra' && i.category_name.toLowerCase() !== 'jus')

  const { data: favoriteList = [], isLoading: favLoading, error: favError } = useQuery({
    queryKey: ["favorites"],
  ...
  const isFavorite = (id) =>
    favoriteList?.some((item) => item.id === id);

  if (productsLoading || favLoading)
    return <Loading />;

  if (productsError)
    return <div className={styles.error}>Error: {productsError.message}</div>;

  if (favError)
    return <div className={styles.error}>Error: {favError.message}</div>;
        ...
        <div key={product.id} className={styles.Cart}>
          ...
          <div className={styles.PriceWrapper}>
            <span dir="ltr" className={styles.ProductPrice}>
              <bdi>درهم</bdi> {product.discount ? (product.price * (1 - product.discount / 100)).toFixed(2) : product.price}
            </span>
